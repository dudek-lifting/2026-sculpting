...
const programBlocks = {
  1: {
    ...
    days: {
      ...
      4: { title: "Arms & Traps", lifts: ["Alt Hammer Curls","Close-Grip Bench Press","Barbell Shrugs","Skull Crushers","Barbell Curls","Single-Arm Cable Pushdowns","Underhand Pulldowns"] }
    }
  },
...
function renderBlock(block, btn) {
  ...
  wc.className = "mb-3 text-center";
  wc.innerHTML = `
    <label class="form-label fw-semibold" style="color:#f3c66b;">Week Start (optional)</label>
    <input type="text" class="form-control form-control-sm mx-auto" style="max-width:180px;"
      placeholder="MM/DD/YYYY"
      value="${localStorage.getItem("weekStart-"+block) || ""}">
  `;
  wc.querySelector("input").oninput = e =>
    localStorage.setItem("weekStart-"+block, e.target.value);
  phaseContent.appendChild(wc);
